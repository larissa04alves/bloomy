      ...
      {records.length === 0 ? (
        ...
        <div className="flex flex-col gap-2.5">
          {records.map((n) => (
            <article
              key={n.id}
              className="rounded-card bg-white p-3.5 shadow-card-sm"
            >
              <div className="flex items-center justify-between">
                ...
                {n.mood ? (
                  <MoodFaceIcon
                    mood={n.mood}
                    size={18}
                    color={MOOD_RECORD_COLOR[n.mood]}
                  />
                ) : null}
              </div>
              <p className="mt-1 whitespace-pre-wrap wrap-break-word text-sm text-[#6b6386]">
                {n.note}
              </p>
            </article>
          ))}
