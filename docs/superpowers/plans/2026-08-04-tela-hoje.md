...
- **Rodar testes de `apps/web`.** A flag `--conditions react-server` (que neutraliza `server-only`) vive no script, não no `bun test`:
  - suíte inteira: `cd apps/web && bun run test`
  - um arquivo só: `cd apps/web && bun test --conditions react-server <path>`
  - `bun test <path>` **cru não funciona** — sem a condição, os imports de `server-only` derrubam 8 testes pré-existentes de outros módulos.
  Onde os passos abaixo escreverem `bun test <path>`, leia como a forma com `--conditions react-server`. `bun check-types` roda da raiz.
...
describe("periodFor (fuso America/Sao_Paulo)", () => {
  ...
  it("meia-noite BR é noite (pega o hourCycle: '24' quebraria isso)", () => {
...
export function periodFor(now: Date = new Date()): DayPeriod {
  ...
  // O limite inferior é obrigatório: sem ele a madrugada (0–4) cairia em "afternoon".
...
Run: `cd apps/web && bun test "src/app/(app)/home/hooks/format.test.ts"`
Expected: PASS (11 testes: 2 de saudação, 3 de data, 2 de consulta, 4 de treino)
